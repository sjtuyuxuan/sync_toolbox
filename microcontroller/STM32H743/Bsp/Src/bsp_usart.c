...
void MX_USARTx_Init(void)
{
  ...
  husartx.Init.OneBitSampling  = UART_ONE_BIT_SAMPLE_DISABLE;
  husartx.Init.Prescaler       = UART_PRESCALER_DIV1;
  husartx.Init.FIFOMode        = UART_FIFOMODE_DISABLE;
  husartx.Init.TXFIFOThreshold = UART_TXFIFO_THRESHOLD_1_8;
  husartx.Init.RXFIFOThreshold = UART_RXFIFO_THRESHOLD_1_8;
  husartx.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
...
void MX_USARTx_IRQHandler(void)
{
	...
	{
		uint8_t data;
		data=READ_REG(husartx.Instance->RDR);
		if(RxCount==0)
		{
